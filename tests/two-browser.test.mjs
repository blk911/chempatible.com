...
await member.window.eval('refreshLive()');assert.equal(member.window.eval('s.phase'),'firstResults');prospect.window.eval('goHome()');assert.equal(prospect.window.eval('s.view'),'dashboard');assert.match(prospect.window.document.querySelector('.socialMemberHeader').textContent,/UNLOCK MY NEXT FIVE/);assert.equal(prospect.window.document.querySelectorAll('.chempatStory').length,1);prospect.window.eval("navigate('results','prospect')");
...
assert.equal(member.window.document.querySelectorAll('.chempatStory').length,2);
...
assert.equal(prospect.window.document.querySelectorAll('.chempatStory').length,2);
...
await member.window.eval('acceptRequest()');assert.equal(member.window.eval('s.view'),'dashboard');await prospect.window.eval('refreshLive()');assert.equal(prospect.window.eval('s.view'),'dashboard');prospect.window.eval("selectChempat('first')");assert.match(prospect.window.document.querySelector('.connectionFocus').textContent,/Private Chat open/i);prospect.window.eval('openConversation()');assert.equal(prospect.window.eval('s.view'),'conversation');
