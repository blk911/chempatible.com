...
let src=fs.readFileSync(new URL('../api/email.mjs',import.meta.url),'utf8').replace("import {neon} from '@neondatabase/serverless';",'const neon=()=>globalThis.__sql;').replace("import {ensureConnectionSchema} from './connection-schema.mjs';",'const ensureConnectionSchema=async()=>{};');globalThis.__sql=sql;process.env.DATABASE_URL='postgres://test';process.env.SENDGRID_API_KEY='test';process.env.CHEMPAT_FROM_EMAIL='hello@chempatible.com';globalThis.fetch=async(u,opt)=>{sent.push(JSON.parse(opt.body));return {ok:true,status:202}};
...
const r=await api.fetch(req),result=await r.json();assert.equal(r.status,200);assert.match(result.id,/^[a-f0-9]{64}$/);assert(connected);assert(invitation);assert.equal(sent[0].personalizations[0].to[0].email,'mike@example.com');assert.equal(sent[0].from.email,'hello@chempatible.com');assert.match(sent[0].content[1].value,/I’M IN · PLAY THE FIVE/);assert.match(sent[0].subject,/Your move/);assert.equal(sent[0].attachments[0].content_id,'inviter-photo');console.log('Email send payload and invitation storage passed');
