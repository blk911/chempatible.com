...
const testPlans = [
  { slug: 'love-language', name: 'Love Language', order: '01', theme: 'How I give and receive affection', detail: 'A shared discovery about gestures, attention, words, time, and care.' },
...
function questionPage(testSlug) {
  if (testSlug) {
    ...
    if (test) return `<div class="question-page"><a class="back-link" href="/admin#questions" data-question-link>← ALL QUESTIONS & SECRETS</a><div class="question-hero"><span class="eyebrow">DEEPER SECRETS / ${test.order}</span><h2>${test.name}</h2><p>${test.theme}</p><span class="test-status">DISCOVERY DRAFT · NOT LIVE</span></div><div class="test-brief"><h3>What this reveals</h3><p>${test.detail}</p><h3>How two people get here</h3><ol><li>They answer each other’s five questions.</li><li>Each privately chooses MORE.</li><li>One invites the other to explore this together.</li><li>Both independently choose their answers. Results reveal to both together.</li></ol><p class="question-caveat">Questions and the shared reveal still need to be designed. This link opens the discovery brief, not a live game.</p></div></div>`;
  }
  ...
  return `<div class="question-page"><div class="question-hero"><span class="eyebrow">THE INTRINSIC DIG</span><h2>Questions that open a connection.</h2><p>Fifty questions across ten intrinsic areas. Choose and edit the first ten in Templates.</p><div class="sequence"><span>YOUR 5</span><span>THEIR 5</span><span>PRIVATE MORE / THANK YOU</span><span>MORE SECRETS</span></div></div><section class="question-section"><div class="section-heading"><div><span class="eyebrow">01 / QUESTION BANK</span><h3>All 50 questions</h3></div><span class="count-badge">50 DRAFTED</span></div><div class="question-controls"><label for="questionSearch">Find a question<input id="questionSearch" type="search" placeholder="Search topic or wording"></label></div><div id="questionList" class="question-list">${cards}</div><p id="emptyQuestions" class="question-caveat" hidden>No questions match that search.</p></section><section class="question-section" id="wildcardTests"><div class="section-heading"><div><span class="eyebrow">02 / AFTER MUTUAL MORE</span><h3>More secrets to uncover</h3><p>Both choose their answers independently; their discoveries reveal together.</p></div></div><div class="test-list">${tests}</div></section></div>`;
}
